...
/*!
 * Generated using the Bootstrap Customizer (http://getbootstrap.com/customize/?id=235f9ccea9478534d725)
 * Config saved to config.json and https://gist.github.com/235f9ccea9478534d725
 */
...
/*
 * ========================================================================
 * Bootstrap: alert.js v3.3.5 http://getbootstrap.com/javascript/#alerts
 * ========================================================================
...
+function($) {
	...
	Alert.prototype.close = function(e) {
		...
		$.support.transition && $parent.hasClass('fade') ? $parent.one(
				'bsTransitionEnd', removeElement).emulateTransitionEnd(
				Alert.TRANSITION_DURATION) : removeElement()
	}
...
/*
 * ========================================================================
 * Bootstrap: button.js v3.3.5 http://getbootstrap.com/javascript/#buttons
 * ========================================================================
 * Copyright 2011-2015 Twitter, Inc. Licensed under MIT
 * (https://github.com/twbs/bootstrap/blob/master/LICENSE)
 * ========================================================================
	...
	Button.prototype.toggle = function() {
		...
		if ($parent.length) {
			...
			if ($input.prop('type') == 'radio') {
				...
				if (($input.prop('checked')) !== this.$element
						.hasClass('active'))
					changed = false
...
+function($) {
	...
	var Carousel = function(element, options) {
		...
		this.options.keyboard
				&& this.$element.on('keydown.bs.carousel', $.proxy(
						this.keydown, this))

		this.options.pause == 'hover'
				&& !('ontouchstart' in document.documentElement)
				&& this.$element.on('mouseenter.bs.carousel',
						$.proxy(this.pause, this)).on('mouseleave.bs.carousel',
	...
	Carousel.prototype.to = function(pos) {
		var that = this
		var activeIndex = this.getItemIndex(this.$active = this.$element
				.find('.item.active'))
		...
		return this.slide(pos > activeIndex ? 'next' : 'prev', this.$items
				.eq(pos))
	}
	...
	Carousel.prototype.slide = function(type, next) {
		...
		if (this.$indicators.length) {
			...
			var $nextIndicator = $(this.$indicators.children()[this
					.getItemIndex($next)])
	...
	function Plugin(option) {
		return this.each(function() {
			...
			var options = $.extend({}, Carousel.DEFAULTS, $this.data(),
...
/*
 * ========================================================================
 * Bootstrap: dropdown.js v3.3.5 http://getbootstrap.com/javascript/#dropdowns
 * ========================================================================
 * Copyright 2011-2015 Twitter, Inc. Licensed under MIT
 * (https://github.com/twbs/bootstrap/blob/master/LICENSE)
 * ========================================================================
...
+function($) {
	...
	Dropdown.prototype.toggle = function(e) {
		...
		if (!isActive) {
			if ('ontouchstart' in document.documentElement
				...
				// delegate
				$(document.createElement('div')).addClass('dropdown-backdrop')
	...
	$(document).on('click.bs.dropdown.data-api', clearMenus).on(
			...
			toggle, Dropdown.prototype.keydown).on(
			'keydown.bs.dropdown.data-api', '.dropdown-menu',
...
+function($) {
	...
	Modal.prototype.show = function(_relatedTarget) {
		...
		this.backdrop(function() {
			...
			}).emulateTransitionEnd(Modal.TRANSITION_DURATION) : that.$element
	...
	Modal.prototype.hide = function(e) {
		...
		this.$element.removeClass('in').off('click.dismiss.bs.modal').off(
		...
		$.support.transition && this.$element.hasClass('fade') ? this.$element
				...
				.emulateTransitionEnd(Modal.TRANSITION_DURATION) : this
				.hideModal()
	}
	...
	Modal.prototype.backdrop = function(callback) {
		...
		if (this.isShown && this.options.backdrop) {
			...
			this.$backdrop = $(document.createElement('div')).addClass(
					'modal-backdrop ' + animate).appendTo(this.$body)
		...
		this.$element
				.css({
					...
					paddingRight : this.bodyIsOverflowing
							&& !modalIsOverflowing ? this.scrollbarWidth : ''
				})
	}
	...
	Modal.prototype.checkScrollbar = function() {
		...
		if (!fullWindowWidth) { // workaround for missing window.innerWidth in
								// IE8
			var documentElementRect = document.documentElement
	...
	function Plugin(option, _relatedTarget) {
		return this.each(function() {
			...
			var options = $.extend({}, Modal.DEFAULTS, $this.data(),
					typeof option == 'object' && option)
...
/*
 * ========================================================================
 * Bootstrap: tooltip.js v3.3.5 http://getbootstrap.com/javascript/#tooltip
...
+function($) {
	...
	Tooltip.prototype.init = function(type, element, options) {
		...
		this.$viewport = this.options.viewport
				&& $($.isFunction(this.options.viewport) ? this.options.viewport
	...
	Tooltip.prototype.enter = function(obj) {
		var self = obj instanceof this.constructor ? obj : $(obj.currentTarget)
				.data('bs.' + this.type)

		if (!self) {
			self = new this.constructor(obj.currentTarget, this
					.getDelegateOptions())
		...
		if (this.hasContent() && this.enabled) {
			...
			if (autoPlace) {
				...
				placement = placement == 'bottom'
						&& pos.bottom + actualHeight > viewportDim.bottom ? 'top'
						: placement == 'top'
								&& pos.top - actualHeight < viewportDim.top ? 'bottom'
								: placement == 'right'
										&& pos.right + actualWidth > viewportDim.width ? 'left'
										: placement == 'left'
												&& pos.left - actualWidth < viewportDim.left ? 'right'
												: placement
			...
			$.support.transition && this.$tip.hasClass('fade') ? $tip.one(
					'bsTransitionEnd', complete).emulateTransitionEnd(
					Tooltip.TRANSITION_DURATION) : complete()
		}
	}
	...
	Tooltip.prototype.replaceArrow = function(delta, dimension, isVertical) {
		this.arrow().css(isVertical ? 'left' : 'top',
				50 * (1 - delta / dimension) + '%').css(
				isVertical ? 'top' : 'left', '')
	}
	...
	Tooltip.prototype.hide = function(callback) {
		...
		function complete() {
			if (that.hoverState != 'in')
				$tip.detach()
			that.$element.removeAttr('aria-describedby').trigger(
					'hidden.bs.' + that.type)
			callback && callback()
		}
		...
		$.support.transition && $tip.hasClass('fade') ? $tip.one(
				'bsTransitionEnd', complete).emulateTransitionEnd(
				Tooltip.TRANSITION_DURATION) : complete()
	...
	Tooltip.prototype.toggle = function(e) {
		...
		if (e) {
			...
			if (!self) {
				self = new this.constructor(e.currentTarget, this
						.getDelegateOptions())
...
/*
 * ========================================================================
 * Bootstrap: popover.js v3.3.5 http://getbootstrap.com/javascript/#popovers
 * ========================================================================
 * Copyright 2011-2015 Twitter, Inc. Licensed under MIT
 * (https://github.com/twbs/bootstrap/blob/master/LICENSE)
 * ========================================================================
...
+function($) {
	...
	Popover.prototype.setContent = function() {
		...
		// IE8 doesn't accept hiding via the `:empty` pseudo selector, we have
		// to do
...
+function($) {
	...
	Tab.prototype.activate = function(element, container, callback) {
		...
		var transition = callback
				&& $.support.transition
				&& ($active.length && $active.hasClass('fade') || !!container
						.find('> .fade').length)
		...
		function next() {
			...
			element.addClass('active').find('[data-toggle="tab"]').attr(
					'aria-expanded', true)
			...
			if (element.parent('.dropdown-menu').length) {
				element.closest('li.dropdown').addClass('active').end().find(
		...
		$active.length && transition ? $active.one('bsTransitionEnd', next)
...
/*
 * ========================================================================
 * Bootstrap: affix.js v3.3.5 http://getbootstrap.com/javascript/#affix
 * ========================================================================
 * Copyright 2011-2015 Twitter, Inc. Licensed under MIT
 * (https://github.com/twbs/bootstrap/blob/master/LICENSE)
 * ========================================================================
...
+function($) {
	...
	var Affix = function(element, options) {
		...
		this.$target = $(this.options.target).on('scroll.bs.affix.data-api',
	...
	Affix.prototype.getState = function(scrollHeight, height, offsetTop,
		...
		if (this.affixed == 'bottom') {
			if (offsetTop != null)
				return (scrollTop + this.unpin <= position.top) ? false
						: 'bottom'
	...
	Affix.prototype.checkPosition = function() {
		...
		if (this.affixed != affix) {
			...
			this.$element.removeClass(Affix.RESET).addClass(affixType).trigger(
					affixType.replace('affix', 'affixed') + '.bs.affix')
		}
...
/*
 * ========================================================================
 * Bootstrap: collapse.js v3.3.5 http://getbootstrap.com/javascript/#collapse
 * ========================================================================
 * Copyright 2011-2015 Twitter, Inc. Licensed under MIT
 * (https://github.com/twbs/bootstrap/blob/master/LICENSE)
 * ========================================================================
...
+function($) {
	...
	Collapse.prototype.show = function() {
		...
		this.$element.one('bsTransitionEnd', $.proxy(complete, this))
				.emulateTransitionEnd(Collapse.TRANSITION_DURATION)[dimension]
				(this.$element[0][scrollSize])
	}
	...
	Collapse.prototype.hide = function() {
		...
		this.$element[dimension](0).one('bsTransitionEnd',
				$.proxy(complete, this)).emulateTransitionEnd(
	...
	Collapse.prototype.getParent = function() {
		return $(this.options.parent).find(
				'[data-toggle="collapse"][data-parent="' + this.options.parent
						+ '"]').each(
				$.proxy(function(i, element) {
					var $element = $(element)
					this.addAriaAndCollapsedClass(
							getTargetFromTrigger($element), $element)
				}, this)).end()
	}
...
/*
 * ========================================================================
 * Bootstrap: scrollspy.js v3.3.5 http://getbootstrap.com/javascript/#scrollspy
 * ========================================================================
 * Copyright 2011-2015 Twitter, Inc. Licensed under MIT
 * (https://github.com/twbs/bootstrap/blob/master/LICENSE)
 * ========================================================================
...
+function($) {
	...
	ScrollSpy.prototype.clear = function() {
		$(this.selector).parentsUntil(this.options.target, '.active')
				.removeClass('active')
	}
